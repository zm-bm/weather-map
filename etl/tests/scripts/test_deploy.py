...
def _static_artifact_root(tmp_path: Path) -> Path:
    ...
    (root / "radio" / "playlist.json").write_text("{}", encoding="utf-8")
    (root / "radio" / "track.mp3").write_bytes(b"mp3")
    return root
