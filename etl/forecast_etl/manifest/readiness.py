...
class PublishReadiness:
    """Resolved publish inputs after readiness checks."""
...
def check_publish_readiness(
    *,
    ...
    if not frames:
        ...
        return PublishReadiness(ready=False)

    if not resolved_artifact_ids:
        print("Publish not ready: workload.artifacts is empty")
        return PublishReadiness(ready=False, frames=frames)
    ...
    if resolved_run_id is None:
        ...
        return PublishReadiness(ready=False, frames=frames, artifact_ids=resolved_artifact_ids)
