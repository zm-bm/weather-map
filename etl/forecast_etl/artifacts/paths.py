...
This module defines:
- WorkItem: unit of execution identity (dataset + cycle + frame + optional artifact_id)
...
class WorkItem:
    """Execution identity for one cycle, frame, and optional artifact."""
    ...
    def __post_init__(self) -> None:
        _ = _safe_segment(self.dataset_id)
        ...
        if len(self.frame_id) != 3 or not self.frame_id.isdigit():
            raise ValueError(f"frame_id must be FFF (3 digits), got: {self.frame_id!r}")
...
class ArtifactPaths:
    ...
    def _dataset_cycle_frame_artifact_parts(self, item: WorkItem) -> tuple[str, str, str, str]:
        dataset_id = _safe_segment(item.dataset_id)
        cycle = _safe_segment(item.cycle)
        validate_run_id(item.run_id)
        frame_id = _safe_segment(item.frame_id)
        ...
        return dataset_id, cycle, frame_id, artifact_id

    def run_prefix_key(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
    ...
    def run_prefix_uri(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        """URI prefix for one immutable ETL run."""

        return join_uri(self.artifact_root_uri, [self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id)])
    ...
    def run_metadata_uri(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        ...
        return join_uri(self.artifact_root_uri, [self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id), "run.json"])

    def run_pipeline_config_uri(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        """Pipeline config snapshot URI for one run."""

        return join_uri(
            self.artifact_root_uri,
            [self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id), "config", "pipeline_config.json"],
        )

    def run_forecast_catalog_uri(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        """Forecast catalog snapshot URI for one run."""

        return join_uri(
            self.artifact_root_uri,
            [self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id), "config", "forecast_catalog.json"],
        )
    ...
    def success_marker_uri(self, item: WorkItem) -> str:
        ...
        dataset_id, cycle, frame_id, artifact_id = self._dataset_cycle_frame_artifact_parts(item)
        path = [
            self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=item.run_id),
            "status",
            artifact_id,
            f"{frame_id}{SUCCESS_MARKER_SUFFIX}",
        ]
        return join_uri(self.artifact_root_uri, path)

    def output_field_payload_uri(self, item: WorkItem, *, dtype: str) -> str:
        """Field payload URI: {root}/runs/{dataset_id}/{cycle}/{run_id}/fields/{frame_id}/{artifact}.field.<dtype>.bin"""
        dataset_id, cycle, frame_id, artifact_id = self._dataset_cycle_frame_artifact_parts(item)
        path = [
            self.field_payload_root_key(dataset_id=dataset_id, cycle=cycle, run_id=item.run_id),
            frame_id,
            self.field_payload_filename(artifact_id=artifact_id, dtype=dtype),
        ]
        return join_uri(self.artifact_root_uri, path)

    def field_payload_root_key(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        """Relative public field payload root key for one run."""

        return "/".join([self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id), "fields"])
    ...
    def logs_uri(self, item: WorkItem) -> str:
        """Log file URI for given WorkItem: {root}/runs/{dataset_id}/{cycle}/{run_id}/logs/{artifact}/{frame_id}.log"""
        dataset_id, cycle, frame_id, artifact_id = self._dataset_cycle_frame_artifact_parts(item)
        path = [
            self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=item.run_id),
            "logs",
    ...
    def success_marker_uri_parts(
        ...
        *,
        ...
        dataset_id = _safe_segment(dataset_id)
        cycle = _safe_segment(cycle)
        run_id = validate_run_id(run_id)
        frame_id = _safe_segment(frame_id)
        artifact_id = _safe_segment(artifact_id)
        path = [
            self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id),
            "status",
            artifact_id,
            f"{frame_id}{SUCCESS_MARKER_SUFFIX}",
        ]
    ...
    def status_prefix_uri(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        ...
        path = [self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id), "status"]
        return join_uri(self.artifact_root_uri, path)

    def published_marker_uri(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        """Marker written by publish role when the run is published."""
        path = [self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id), PUBLISHED_MARKER_FILENAME]
        return join_uri(self.artifact_root_uri, path)

    def run_manifest_uri(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        """Canonical internal manifest URI for one immutable run."""

        path = [self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id), "manifest.json"]
        return join_uri(self.artifact_root_uri, path)
    ...
    def validation_report_uri(self, *, dataset_id: str, cycle: str, run_id: str) -> str:
        ...
        path = [self.run_prefix_key(dataset_id=dataset_id, cycle=cycle, run_id=run_id), VALIDATION_REPORT_FILENAME]
        return join_uri(self.artifact_root_uri, path)
    ...
    def cycle_current_pointer_uri(self, *, dataset_id: str, cycle: str) -> str:
        """Cycle current pointer URI: {root}/manifests/{dataset_id}/cycles/{cycle}/current.json"""

        path = ["manifests", _safe_segment(dataset_id), "cycles", _safe_segment(cycle), "current.json"]
        return join_uri(self.artifact_root_uri, path)

    def manifest_prefix_uri(self, *, dataset_id: str) -> str:
        """Prefix under which public dataset manifests and pointers live."""
        path = ["manifests", _safe_segment(dataset_id)]
        return join_uri(self.artifact_root_uri, path)

    def manifest_latest_uri(self, *, dataset_id: str) -> str:
        """Canonical latest manifest pointer alias: {root}/manifests/{dataset_id}/latest.json"""
        path = ["manifests", _safe_segment(dataset_id), "latest.json"]
        return join_uri(self.artifact_root_uri, path)

    def data_manifest_uri(self) -> str:
        """Frontend data manifest URI: {root}/manifests/data-manifest.json"""
        path = ["manifests", DATA_MANIFEST_FILENAME]
        return join_uri(self.artifact_root_uri, path)
