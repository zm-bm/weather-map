...
class ManifestInspectTest(unittest.TestCase):
    ...
    def test_manifest_cycle_from_key_rejects_latest_and_non_matching_keys(self) -> None:
        self.assertIsNone(manifest_cycle_from_key(dataset_id="gfs", key="manifests/gfs/latest.json"))
        self.assertIsNone(manifest_cycle_from_key(dataset_id="gfs", key="manifests/gfs/2026051106.json"))
        self.assertIsNone(manifest_cycle_from_key(dataset_id="gfs", key="manifests/icon/2026051106.json"))
        self.assertIsNone(manifest_cycle_from_key(dataset_id="gfs", key="manifests/gfs/not-a-cycle.json"))
        self.assertIsNone(
            manifest_cycle_from_key(
                dataset_id="gfs",
    ...
    def test_manifest_info_read_helpers_use_store_and_artifact_paths(self) -> None:
        with temp_artifact_fixture() as artifacts:
            ...
            infos = list_manifest_infos(store=artifacts.store, paths=artifacts.paths, dataset_id="gfs", limit=1)
            latest = read_latest_manifest_info(store=artifacts.store, paths=artifacts.paths, dataset_id="gfs")
    ...
    def test_latest_pointer_info_and_object_are_dereferenced(self) -> None:
        ...
        with temp_artifact_fixture() as artifacts:
            ...
            infos = list_manifest_infos(store=artifacts.store, paths=artifacts.paths, dataset_id="gfs", limit=1)
            latest = read_latest_manifest_info(store=artifacts.store, paths=artifacts.paths, dataset_id="gfs")
            latest_obj = read_latest_manifest_object(artifact_repo=artifacts.repository, dataset_id="gfs")
