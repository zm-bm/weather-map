"""Prepared GRIB source objects handed from source acquisition to artifact processing."""
...
class PreparedGribSource:
    """GRIB source object consumed by artifact extraction after acquisition."""

    uri: str
    grid_id: str

    @staticmethod
    def grib(*, uri: str, path: Path, grid_id: str) -> "PreparedGribSource":
        """Create a prepared GRIB source backed by one file."""

        return SingleFilePreparedGribSource(uri=uri, path=path, grid_id=grid_id)
    ...
    def grib_collection(
        *,
        ...
        """Create a prepared GRIB source backed by selector-keyed files."""

        return GribCollectionPreparedGribSource(
            uri=uri,
...
class SingleFilePreparedGribSource(PreparedGribSource):
    """Prepared GRIB source where every component reads from the same file."""
...
@dataclass(frozen=True)
class GribCollectionPreparedGribSource(PreparedGribSource):
    """Prepared GRIB source that selects component files by metadata value."""
    ...
    def component_grib_path(
        ...
        *,
        ...
        if not selector_value:
            raise SystemExit(
                f"Artifact {artifact_id}.{component_id} requires {self.selector_key} for GRIB collection source"
            )
