...
from .band_inputs import (
    bands_from_component_bytes,
    expected_derivation_inputs,
    extract_derivation_input_band,
    validate_output_component_ids,
)
from .icon_accumulated_precip import extract_icon_precip_rate_band
...
OverlayFractionFn = Callable[..., tuple[float, float]]


def extract_gfs_precip_type_overlay(
    *,
    ...
    input_items = expected_derivation_inputs(
        artifact=artifact,
        derivation_type=DERIVATION_PRECIP_TYPE_OVERLAY_FROM_GFS,
        expected_input_ids=GFS_PRECIP_TYPE_OVERLAY_INPUT_IDS,
    )
    input_bands = {
        input_item.id: extract_derivation_input_band(
            artifact=artifact,
            grid=grid,
            source=source,
    ...
    return bands_from_component_bytes(artifact=artifact, component_bytes=output_bytes)


def extract_icon_precip_type_overlay(
    *,
    artifact: ArtifactSpec,
    grid: dict[str, Any],
    source: PreparedGribSource,
    ...
    input_bands = {
        input_item.id: extract_icon_precip_rate_band(
            artifact=artifact,
            derivation_type=DERIVATION_PRECIP_TYPE_OVERLAY_FROM_ICON_COMPONENTS,
...
def _precip_type_overlay_bytes(
    *,
    ...
    bands = {
        input_id: _required_band(input_bands, input_id=input_id, artifact_id=artifact_id) for input_id in input_ids
    }
    ...
    for index, values in enumerate(zip(*iterators, strict=True)):
        snow_frac, mix_frac = fraction_fn(*values)
...
def _gfs_precip_overlay_fractions(
    precip_rate: float,
    frozen_percent: float,
    rain_value: float,
    freezing_rain_value: float,
    ice_pellets_value: float,
    snow_value: float,
) -> tuple[float, float]:
    if not math.isfinite(precip_rate):
        return math.nan, math.nan

    intensity_mm_hr = precip_rate * 3600.0
    if intensity_mm_hr < PRECIP_TYPE_OVERLAY_MIN_INTENSITY_MM_HR:
        return 0.0, 0.0

    rain = _category_active(rain_value)
    freezing_rain = _category_active(freezing_rain_value)
    ice_pellets = _category_active(ice_pellets_value)
    snow = _category_active(snow_value)
    ...
    if freezing_rain or ice_pellets:
        ...
        snow_frac = 0.0
        mix_frac = 0.0
    elif math.isfinite(frozen_percent):
        frozen_frac = _clamp(frozen_percent / 100.0, 0.0, 1.0)
...
def _icon_precip_overlay_fractions(
    rain_gsp: float,
    rain_con: float,
    snow_gsp: float,
    snow_con: float,
) -> tuple[float, float]:
    values = (rain_gsp, rain_con, snow_gsp, snow_con)
    if not all(math.isfinite(value) for value in values):
        return math.nan, math.nan

    rain_rate = max(rain_gsp, 0.0) + max(rain_con, 0.0)
    snow_rate = max(snow_gsp, 0.0) + max(snow_con, 0.0)
