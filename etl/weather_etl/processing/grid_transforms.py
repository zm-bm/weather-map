...
from .float32 import float32_array_from_bytes, float32_array_to_little_endian_bytes
...
def regular_grid_downsample_2x_bytes(source_f32_bytes: bytes, *, byte_order: str, grid: dict[str, Any]) -> bytes:
    ...
    source = float32_array_from_bytes(source_f32_bytes, byte_order=byte_order)
    target_grid = regular_grid_downsample_2x_meta(grid)
    nx = int(grid["nx"])
    ny = int(grid["ny"])
    ...
    return float32_array_to_little_endian_bytes(target)
