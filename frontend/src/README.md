...
- `forecast/catalog/*`: parsed and validated frontend catalog for user-facing layers, particle layers, layer groups, labels, color palettes, display ranges, and artifact or derived-source mappings. Catalog entries reference unit and legend behavior contracts owned by `forecast/units` and `forecast/legend`.
...
- `forecast/artifacts/*`: manifest artifact I/O and binary decoding. This module resolves artifact payload refs, fetches and caches frame payload bytes, validates payload size, and decodes scalar or generic vector-component artifact data. Semantic meanings such as wind vectors belong in `forecast/data/loaders`.
...
- `forecast/cache/*`: generic byte-limited memory and IndexedDB cache infrastructure. Keep eviction, byte limits, and pending writes here; forecast-specific cache scope and keys belong with the artifact/data owner using the cache.
