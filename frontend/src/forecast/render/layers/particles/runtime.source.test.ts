...
describe('particle runtime source', () => {
  ...
  it('passes MapLibre projection uniforms to particle drawing', () => {
    expect(RUNTIME_SOURCE).toContain('createProjectionProgramCache')
    expect(RUNTIME_SOURCE).toContain('projectionUniformValues(input)')
    expect(RUNTIME_SOURCE).toContain('state.particleProgramCache?.get(input)')
    expect(PASSES_INDEX_SOURCE).toContain('export type ParticleProjectionUniforms = ProjectionUniformValues')
    expect(TRAIL_PASS_SOURCE).toContain('drawParticleGeometryPass(state, options, projection)')
    expect(PARTICLE_PASS_SOURCE).toContain('...projection')
    expect(PARTICLE_PASS_SOURCE).not.toContain('u_matrix')
    expect(PARTICLE_PASS_SOURCE).not.toContain('u_world_size')
