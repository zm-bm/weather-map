...
describe('pressure contour shader sources', () => {
  it('prefilters raw encoded pressure into an hPa field with coverage', () => {
    ...
    expect(PRESSURE_SMOOTHING_FRAGMENT_SHADER_SOURCE).not.toContain('CONTOUR_EDGE_EPSILON_HPA')
    expect(PRESSURE_SMOOTHING_FRAGMENT_SHADER_SOURCE).not.toContain('MAX_CONTOUR_LEVELS_PER_CELL')
  })
  ...
  it('uses smoothed pressure field marching squares in the main contour shader', () => {
    ...
    expect(PRESSURE_CONTOUR_FRAGMENT_SHADER_SOURCE).not.toContain('PASCALS_PER_HECTOPASCAL')
    expect(PRESSURE_CONTOUR_FRAGMENT_SHADER_SOURCE).not.toContain('SMOOTHING_FULL_WEIGHT')
    expect(PRESSURE_CONTOUR_FRAGMENT_SHADER_SOURCE).not.toContain('nearestContourLevelHpa')
    expect(PRESSURE_CONTOUR_FRAGMENT_SHADER_SOURCE).not.toContain('samplePressureFieldCellBilinear')
    expect(PRESSURE_CONTOUR_FRAGMENT_SHADER_SOURCE).not.toContain('fwidth(pressureHpa)')
    expect(PRESSURE_CONTOUR_FRAGMENT_SHADER_SOURCE).not.toContain('contourPhaseBandAlpha')
    expect(PRESSURE_CONTOUR_FRAGMENT_SHADER_SOURCE).not.toContain('contourPhaseDistanceHpa')
    ...
    expect(PRESSURE_CONTOUR_FRAGMENT_SHADER_SOURCE).not.toContain('weightedEncodedSample')
  })
})
