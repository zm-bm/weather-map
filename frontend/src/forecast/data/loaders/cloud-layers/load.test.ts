...
import { createCloudLayersDataLoad, materializeCloudLayersTimeSlice } from './load'
...
describe('materializeCloudLayersTimeSlice', () => {
  it('keeps low middle high components and derives composite coverage', () => {
    ...
    const load = createCloudLayersDataLoad({
      activeRun: createActiveRunFixture(createManifestFixture()),
      source,
      artifacts: {
        canLoadVectorComponents: () => true,
      } as unknown as ArtifactLoader,
    })
    expect(load?.probeField?.projectTimeSlice(slice)).toBe(slice.coverage)
  })
...
function createCloudLayersSource() {
  const source = createCloudLayerSourceFixture()
