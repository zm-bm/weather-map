...
import { createFieldDataLoad } from './load'
...
function fieldLoad(args: {
  ...
  const source = createLayerDataSource(args.layer ?? layer(args.manifest, args.layerId))
...
describe('createFieldDataLoad', () => {
  it('loads direct artifact fields and applies catalog display metadata', async () => {
    ...
    const frame = await fieldLoad({ manifest }).loadTimeSlice('000')
