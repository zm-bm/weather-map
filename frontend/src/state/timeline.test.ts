...
describe('timeline transport machine', () => {
  ...
  it('dispatches immediately when not debounced and not in flight', () => {
    const state = reduceTimelineState(
      createTimelineState(0),
      {
        type: 'requestHour',
        hourIndex: 1,
        nowMs: 1000,
  ...
  it('queues request during debounce window', () => {
    ...
    const state = reduceTimelineState(initial, {
      type: 'requestHour',
      hourIndex: 2,
      nowMs: 1100,
      debounceMs: 250,
  ...
  it('dispatches queued hour only when debounce elapsed and no in-flight request', () => {
    const readyState = {
      ...createTimelineState(1),
      pendingHourIndex: 2,
      lastDispatchAtMs: 0,
    }
    const dispatched = reduceTimelineState(readyState, {
      type: 'flushPending',
      nowMs: 500,
      debounceMs: 250,
    })
    ...
    const queued = reduceTimelineState(waitingState, {
      type: 'flushPending',
      nowMs: 500,
      debounceMs: 250,
    })
