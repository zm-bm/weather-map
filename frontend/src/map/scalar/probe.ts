...
const probeFrames = new WeakMap<MapLibreMap, ScalarFrameData>()

export function setScalarProbeFrame(map: MapLibreMap, frame: ScalarFrameData) {
  probeFrames.set(map, frame)
}

export function getScalarProbeFrame(map: MapLibreMap): ScalarFrameData | null {
  return probeFrames.get(map) ?? null
}

export function clearScalarProbeFrame(map: MapLibreMap) {
  probeFrames.delete(map)
}
