...
type CreatePressureChannelArgs = {
  artifacts: ArtifactLoader
  activeRun: ActiveForecastRun
}

export function createPressureChannel(
  args: CreatePressureChannelArgs
): ForecastProductChannel<PressureTimeSliceData> | null {
  if (!args.artifacts.canLoadScalar(PRESSURE_SOURCE_ARTIFACT_ID)) return null

  return {
    key: createPressureChannelKey(args.activeRun, PRESSURE_SOURCE_ARTIFACT_ID),
    load: (hourToken) => loadPressureTimeSlice({
