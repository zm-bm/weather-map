...
type ForecastProductPrefetchTask = {
  product: ForecastProductLoad
  hourToken: string
}

type PrefetchForecastProductsArgs = {
...
export async function prefetchForecastProducts(args: PrefetchForecastProductsArgs): Promise<void> {
  const tasks = createForecastProductPrefetchTasks(args)
...
function createForecastProductPrefetchTasks(args: PrefetchForecastProductsArgs): ForecastProductPrefetchTask[] {
  ...
  return hourTokens.flatMap((hourToken) => {
    return args.request.products.map((product) => ({ product, hourToken }))
  })
}
...
async function runPrefetchQueue(args: {
  tasks: ForecastProductPrefetchTask[]
  concurrency: number
  signal: AbortSignal
}): Promise<void> {
  let nextTaskIndex = 0
  ...
  async function runWorker() {
    while (!args.signal.aborted) {
      ...
      try {
        await task.product.load(task.hourToken)
      } catch {
