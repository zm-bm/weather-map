...
Description:
  Submits one AWS Batch worker job per resumable frame by executing the shared
  ETL cycle plan against AWS Batch.
...
Options:
  ...
  --config-file <path>            Config to compare with deployed config. Default: config/pipeline/base.json.
  --catalog-file <path>           Catalog to compare with deployed catalog. Default: config/forecast_catalog.json.
...
Environment defaults:
  ...
  JOB_NAME_PREFIX, SUBMIT_DELAY_SECONDS, DRY_RUN, BACKFILL, SKIP_CONFIG_CHECK,
...
if [[ "$RUN_ID" != "" && ! "$RUN_ID" =~ ^[0-9]{8}T[0-9]{6}Z-[0-9a-f]{8}$ ]]; then
...
if [[ "$SKIP_CONFIG_CHECK" != "true" ]]; then
  read -r CONFIG_BUCKET CONFIG_KEY < <(parse_s3_uri "$PIPELINE_CONFIG_URI")
  read -r CATALOG_BUCKET CATALOG_KEY < <(parse_s3_uri "$FORECAST_CATALOG_URI")
...
cmd=(
  "$PYTHON_BIN" -m forecast_etl.cli submit-aws-cycle
